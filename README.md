...
Excel 文件读取第一张工作表，并会在前 30 行中自动寻找表头。表头可以出现在标题行之后，也不要求从 A 列开始；当前逻辑会识别 `单元`、片名、影院、日期/时间等核心列。

如果表头中存在 `影片中文名` 等字段，系统会按兼容格式处理，并映射成统一字段结构后再参与筛选、导出和冲突检测。官方「第28届上海国际电影节排片表」这类第一行是标题、第二行才是表头的 Excel 文件可直接导入。
